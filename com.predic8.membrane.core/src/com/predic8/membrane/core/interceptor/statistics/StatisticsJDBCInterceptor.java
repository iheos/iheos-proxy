...
public class StatisticsJDBCInterceptor extends AbstractInterceptor {
    private final int CNX_WAIT = 5000;

    private Log log = LogFactory.getLog(StatisticsJDBCInterceptor.class.getName());
    ...
    private final String DEFAULT_QUEUE = "Queue-Agent-MessageSink";
    ...
    public void init() {
        log.info("Enter init() for StatisticsJDBCInterceptor");
     ...
     * @throws Exception
     */
    private void saveExchange(Connection con, Exchange exc, int flag)
            throws Exception {
        if (ignoreGetMethod(exc))
            return;
        if (ignoreNotSoap(exc))
            return;

        JDBCUtil.setData(exc, stat, idGenerated, flag, "",
                getGatewayHCID(flag, exc));
        ...
        if (rc > 0) {
            ...
            try {
                ...
                if (rs != null && rs.next()) {
                    ...
                    sendMessage(brokerName, brokerUserName, brokerPassword,
                            queue, rs.getInt(1));
                            */
                }
            } catch (Exception ex) {
                log.error("sendMessage Queue notification failed! Transaction record "
                        + rs.getInt(1) + " error: " + ex.toString());
            }
            try {
                rs.close();
            } catch (Exception ex) {
     ...
     * skb
     */
    private void sendMessage(String broker, String username, String password, String sQueue, int transactionId) {
    ...
    private void createPreparedStatement(Connection con) throws Exception {
        if (stat == null || stat.getConnection().isClosed())
            stat = con.prepareStatement(statString, Statement.RETURN_GENERATED_KEYS);
    }
